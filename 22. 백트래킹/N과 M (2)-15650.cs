class NM2{
    ...
    private static void Permutation(HashSet<int> nset, int s, int e, int k, string result)
    {
        for (int i = s; i < e + 1; i++)
        {
            if (nset.Contains(i))
            ...
            {
                nset.Add(i);
                HashSet<int> nsetCopy = new HashSet<int>(nset);

                Permutation(nsetCopy, i+1, e,k - 1, result + i + " ");
            }
        }
    }

    public static void nm2(string[] args)
    {
        ...
        Permutation(NSet, 1, NMList[0], NMList[1], "");
